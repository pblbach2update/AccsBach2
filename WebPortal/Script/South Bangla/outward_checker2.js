$(document).ready(function () {
    ...
    var DOM =
    {
        ...
        today: $("#today_date"),
        cbs: {
            account: $("#cu_account_div"),
            inWord: $("#taka"),
            amount: $("#cu_amount_div"),
            date: $("#cu_date_div"),
            payee: $("#cu_payee_div"),
    ...
    function getChequeInfo(cindx) {
        ...
        ws_request.id = cid;
        ws_request.role = ACCS.Outward.WS_Role.checker;
        ws_request.type = ACCS.Outward.WS_RequestType.get_data;


        clearCBSInfo();
        //Webservice Call
        ACCS.Outward.OutwardService.GetChequeInfo(ws_request,
        function (ws_rsp) {
            try {
                ...
                if (ws_rsp.result == ACCS.Outward.WS_OutwardServiceResponseResult.success) {
                    ...
                    if (_curObj.status == ACCS.Outward.WS_ActionStatus.accept) {
                        DOM.cheque.log_details.attr("class", "correct_box");
                        DOM.cheque.rejection_reason.val("");
                    }
                    else if (_curObj.status == ACCS.Outward.WS_ActionStatus.reject) {
                    ...
                    DOM.cbs.account.text(_curObj.account);
                    DOM.cbs.amount.text(Comma(_curObj.amount) + "");
                    DOM.cbs.payee.text(_curObj.payee);
                    DOM.cbs.date.text(_curObj.date);
    ...
    function getAutoValidation() {
        ...
        //Account Validation
        var account = DOM.cbs.account.text();
        if (!isValidBenifAccount(account))
    ...
    function processChequeUpdate(action) {
        ...
        try {
            ...
            if (action == ACCS.Outward.WS_ActionStatus.accept) {
            ...
            ws_request.data = new ACCS.Outward.WS_ChequeInformation();
            ...
            ws_request.data.account = DOM.cbs.account.text();
            ws_request.data.amount = StripComma(DOM.cbs.amount.text()) + "";
            ws_request.data.date = DOM.cbs.date.text();
            ws_request.data.payee = DOM.cbs.payee.text();
            ...
            //Webservice Call
            ACCS.Outward.OutwardService.UpdateChequeInfo(ws_request,
            //Success
            function (ws_rsp) {
                if (ws_rsp.result == ACCS.Outward.WS_OutwardServiceResponseResult.success) {
                    ...
                    else if (action == ACCS.Outward.WS_ActionStatus.reject)
    ...
    $('#yes').click(function () {
        processChequeUpdate(ACCS.Outward.WS_ActionStatus.accept);
    });
    ...
    $('#reject_confirm').click(function () {
        hideRejectModal();
        processChequeUpdate(ACCS.Outward.WS_ActionStatus.reject);
    });

    $('#remake').click(function () {
        processChequeUpdate(ACCS.Outward.WS_ActionStatus.remake);
    });
